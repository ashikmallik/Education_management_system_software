...
// echo $elective_subject1." ",$elective_subject2." ",$elective_subject3." ",$elective_subject4." ";
...
$chekpaymnet="SELECT * FROM `student_addmission` WHERE `user_id` = '$ssc_roll' AND `is_paid` = 1 AND `due_amount` = 0"; 
	$qchekpaymnet = $mysqli->query($chekpaymnet);
	$smsqchekpaymneth=$qchekpaymnet->fetch_assoc();
	$sid = $smsqchekpaymneth['user_id'];
	if(empty($sid)){
