...
<head>
  ...
  $( function() {
	...
	function callpage()
	{
	 document.frmcontent.action="manage_examiner.php";
...
<form name="frmcontent" id="myform" action="manage_examiner_do.php" method="post" enctype="multipart/form-data">
...
<table width="650" border="0" cellspacing="0" cellpadding="0" align="center" class="projectlist">
  ...
  <tr>
    ...
    <td colspan="2" align="center">Teacher Name</td>
    <td colspan="4" align="center">Change Teacher </td>
  ...
  <?php
	...
	$section=$shgstclassshift11['borno_school_section_id'];				
		...
		$gtctmarg="select * from borno_set_examiner where borno_school_id='$schId' AND borno_school_branch_id='$gtfbranchId' AND borno_school_class_id='$studClass' AND borno_school_shift_id='$shiftId' AND borno_school_section_id='$section' AND borno_school_session='$clSess' AND borno_result_term_id='$selTerm' order by borno_result_subject_id asc";

  	$sal=0;
    $qgtctmarg=$mysqli->query($gtctmarg);
	while($shgtctmarg=$qgtctmarg->fetch_assoc()){ $sal++;
  	$subidno=$shgtctmarg['borno_result_subject_id'];
	$techno=$shgtctmarg['borno_teacher_info_id'];
	
  	$assnty112="select * from borno_subject_nine_ten where borno_subject_nine_ten_id='$subidno'";
	$qassnty112=$mysqli->query($assnty112);
	$shassnty112=$qassnty112->fetch_assoc(); 
  
    $gstteacher="select * from borno_teacher_info where borno_teacher_info_id='$techno'";
                        $qgstteacher=$mysqli->query($gstteacher);
                        $shgstteacher=$qgstteacher->fetch_assoc();   
  
  ?>
 <tr>
    <td colspan="2"><?php echo $shassnty112['borno_subject_nine_ten_name']; ?>
      <input type="hidden" name="stuName[]" id="stuName[]" value="<?php echo $shgtctmarg['borno_set_examiner_id']; ?>" />
            <input type="hidden" name="stuRoll[]" id="stuRoll[]" value="<?php echo $shgtctmarg['borno_subject_nine_ten_id']; ?>" /></td>
    <td colspan="2"><?php echo $shgstteacher['borno_teacher_name']; ?></td>        
 ...
 <select name="teacherid[]" id="teacherid[]">
  ...
  ?>
  <tr>
    <td colspan="2"><?php echo $shassnty112['six_eight_subject_name']; ?>
<input type="hidden" name="stuName[]" id="stuName[]" value="<?php echo $shgtctmarg['borno_set_examiner_id']; ?>" />
 ...
 <select name="teacherid[]" id="teacherid[]">
  ...
  {	
	...
	$techno=$shgtctmarg['borno_teacher_info_id'];
 	...
 	$gsubjName="select * from borno_result_subject where borno_result_subject_id='$subidno'";
  ...
  ?>
  <tr>
    <td colspan="2"><?php echo $shgsubjName['borno_school_subject_name']; ?>
      <input type="hidden" name="stuName[]" id="stuName[]" value="<?php echo $shgtctmarg['borno_set_examiner_id']; ?>" />
            <input type="hidden" name="stuRoll[]" id="stuRoll[]" value="<?php echo $shgtctmarg['borno_result_subject_id']; ?>" /></td>
    <td colspan="2"><?php echo $shgstteacher['borno_teacher_name']; ?>
</td>            
    <td colspan="4" align="center">
 <select name="teacherid[]" id="teacherid[]">
      <option value=""> --Select-- </option>
