...
*/
	...
	if($smsType==1){
		...
		$totcountSMS=strlen($message1);
			...
			if($totcountSMS<=160) { $sendTotalSMS=1;  }
			else if($totcountSMS>=161 AND $totcountSMS<=310 ) { $sendTotalSMS=2;  }
			else if($totcountSMS>=311 AND $totcountSMS<=450 ) { $sendTotalSMS=3;  }
	...
	$gtstudent="select * from borno_teacher_info where borno_teacher_info_id='$stdid'";
	...
	while($shroll=$qgtstudent->fetch_assoc()){ 
			...
			$schId=$shroll['borno_school_id'];
			$stsess=date(Y);
			$To=$shroll['borno_teacher_phone'];			
...
$bornosentSMS="INSERT INTO `borno_sent_sms` (`borno_school_id`,`borno_student_info_id`,`borno_school_branch_id`,`borno_school_class_id`,`borno_school_shift_id`,`borno_school_section_id`,`borno_school_session`,`borno_school_roll`,`borno_school_student_name`, `borno_sent_sms_phone`,`borno_sms_status`,`borno_sms_type`, `borno_sent_sms_message`,`borno_sms_date`,`borno_sms_time`,`total_sms_count`,`bangla_message`) 
				VALUES ('$schId','$stdid','$branchid','0','$shiftid','0','$stsess','0','$name', '$To','Teacher SMS','$smsType', '$message1','$smsdate','$smstime','1','$message1')";
...
$utf8 = $message1;
		...
		if($totcountSMS<=70) { echo $sendTotalSMS=1;  }
		else if($totcountSMS>=71 AND $totcountSMS<=130 ) { echo $sendTotalSMS=2;  }
		else if($totcountSMS>=131 AND $totcountSMS<=196 ) { echo $sendTotalSMS=3;  }
		else if($totcountSMS>=197 AND $totcountSMS<=260 ) { echo $sendTotalSMS=4;  }
		else if($totcountSMS>=261 AND $totcountSMS<=330 ) { echo $sendTotalSMS=5;  }
		else if($totcountSMS>=331 AND $totcountSMS<=390 ) { echo $sendTotalSMS=6;  }	
