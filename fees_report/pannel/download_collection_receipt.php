...
//echo $fdate;
  ...
  $result ="SELECT * FROM `student_fees` WHERE STR_TO_DATE(`receive_date`, '%d/%m/%Y') BETWEEN STR_TO_DATE('$fdate', '%d/%m/%Y') AND STR_TO_DATE('$tdate', '%d/%m/%Y')  group by `student_id` ORDER BY `receive_date` ASC";
...
$totalamount=0;		
$total = array();		
 $result2 ="SELECT * FROM `student_fees` where `money_receipt_no`='$memo'";
