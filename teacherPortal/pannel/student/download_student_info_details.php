...
function mycell($w,$h,$x,$t){
    ...
    if($len>22){
        $txt = str_split($t,22);
        $this->setX($x);
        $this->cell($w,$fir,$txt[0],'','C','');
        $this->setX($x);
        $this->cell($w,$sec,$txt[1],'','C','');
        $this->setX($x);
...
$pdf->AddPage('L');
	...
	//	$pdf->Cell(50,5,$techname,1);
		$x= $pdf->getX();
        $pdf->mycell($w,$h,$x,$fname);
	//	$pdf->Cell(50,5,$fname,1);
		$x= $pdf->getX();
        $pdf->mycell($w,$h,$x,$mname);
	//	$pdf->Cell(50,5,$mname,1);
		$pdf->Cell(25,10,$dob,1);
		$pdf->Cell(25,10,$blg,1);	
		$pdf->Cell(20,10,$religion,1);
		$pdf->Cell(30,10,$techPhone,1);
		   $pdf->Ln();
}
