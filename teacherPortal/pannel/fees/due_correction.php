  ...
  <tr style="background-color: #75cbd6;">
    ...
    <td ><select name="fiscalid" id="fiscalid" onchange="callpage();">
      ...
      <?php } ?>
  		...
  		$gtduelist="SELECT `student_id`,`borno_school_section_name`,`borno_school_roll`,`borno_school_student_name`,`due_amount` FROM `student_fees` AS sf 
                     ...
                     WHERE `fiscal_year_details_id` <= '$yeardetails' AND `fiscal_year_id`='$fiscalid' AND si.`borno_school_class_id` = '$studClass' AND si.borno_school_branch_id='$gtfbranchId'   AND `due_amount` != 0 AND `fees_head_id` = 4
                     GROUP BY `student_id` ORDER BY si.`borno_school_section_id`,`borno_school_roll`";
		$sl=0;
...
<tr>
    ...
    <input type="hidden" name="studId[]" id="studId[]" value="<?php echo $shgtduelist['student_id']; ?>" /></td>
     <td width="16%;" align="center"><?php echo $shgtduelist['borno_school_section_name']; ?></td>
     <td width="16%;" align="center"><?php echo $shgtduelist['borno_school_roll']; ?></td>
