...
$pdf->SetFont('Arial','',10);
		...
		$pdf->Cell(30,5,'Total Student',1,0,"C");
				...
				$data1="select * from borno_student_info where borno_school_id='$schId' AND borno_school_session='$stsess' AND borno_school_class_id='$stclass' group by borno_school_shift_id,borno_school_section_id asc";
					$qdata1=$mysqli->query($data1);
					$sl=0;
					while($showdata1=$qdata1->fetch_assoc()){ $sl++;
					
					$stshift=$showdata1['borno_school_shift_id'];
					$stsection=$showdata1['borno_school_section_id'];
		
	
					$gtsection1="select * from borno_student_info where borno_school_id='$schId' AND borno_school_session='$stsess' AND borno_school_section_id='$stsection'";
