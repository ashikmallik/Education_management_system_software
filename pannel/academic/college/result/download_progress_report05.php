...
function Header()
{
    ...
    $this->Image($finalSchoolLogo1,15,17,25,25);
...
if($group==1)
{
$pdf->Cell(80,5,"Group : Science",0,0,"L");    
}  
elseif($group==2)
{
$pdf->Cell(80,5,"Group : Business Studies",0,0,"L");    
} 
elseif($group==3)
{
$pdf->Cell(80,5,"Group : Humanities",0,0,"L");    
} 
$pdf->SetFont('Arial','',11);
...
$numbertype ="select * from borno_result_number_type where borno_school_session='$stsess' AND borno_school_id='$schId' AND borno_school_class_id='$studClass'";
		...
		$type3=substr($sqnumbertype['borno_school_number_type3'],0,4);
		...
		$pdf->Cell(45,5,"Subject Name",1);
		$pdf->Cell(15,5,"Full Mark",1,0,"C");
		
		
		$pdf->Cell(10,5,$type1,1,0,"C");
		$pdf->Cell(10,5,$type2,1,0,"C");
		$pdf->Cell(10,5,$type3,1,0,"C");
		$pdf->Cell(18,5,"CQ Conv",1,0,"C");
		$pdf->Cell(18,5,"MCQ Conv",1,0,"C");
		$pdf->Cell(18,5,"CT Conv",1,0,"C");
...
$sl=0;
	...
	//foreach($row as $column)
		...
		$stN4=$row['temp_res_number_type1_conv'];
		$stN5=$row['res_number_type2_conv'];
		$stN6=$row['res_number_type3_conv'];
		$stName=round($row['res_total_conv']);
		$stphone=$row['res_gpa'];
		$stgp=$row['res_lg'];
		$subpare=$row['pare'];	
		...
		$gtsubject ="select * from borno_subject_eleven_twelve where borno_subject_eleven_twelve_id=$stN";
					...
					$fsstgtsubject=substr($stgtsubject['borno_subject_eleven_twelve_name'],0,22);

		if($schId==65 AND $fsstgtsubject!="ICT"){$fsstgtsubject=substr($fsstgtsubject,0,-2);}
		$pdf->Cell(45,5,$fsstgtsubject,1);
...
$subjectdetails ="select * from borno_result_eleven_twelve_subject_details where borno_school_session='$stsess' AND borno_school_id='$schId' AND borno_school_class_id='$studClass' AND borno_result_term_id='$term' AND borno_subject_eleven_twelve_id='$stN'";
		...
		$pdf->Cell(15,5,$fsstgtsubject1,1,0,"C");
		if($subtype1!=0)
		{		
		$pdf->Cell(10,5,$stN1,1,0,"C");
		}
		else
		{
		$pdf->Cell(10,5,'-',1,0,"C");
...
$result22 = "select * from borno_height_mark_class11_12 where borno_school_class_id='$studClass' AND borno_school_branch_id='$branchId' AND borno_school_id='$schId' AND borno_school_shift_id='$shiftId' AND borno_school_section_id='$section' AND borno_school_session='$stsess' AND borno_result_term_id='$term' AND borno_school_stud_group='$group' AND borno_result_exam_year='$styear' AND borno_subject_eleven_twelve_id='$stN'";
				...
				$fssthigh=round($row22['getmaxmark']);	
...
$pdf->SetFont('Arial','',12);
		...
		$pdf->Cell(144,6,"GrandTotal / GPA With 4th Subject",1);
...
$pdf->Ln();
$pdf->Cell(144,6,"GrandTotal / GPA Without 4th Subject",1);
