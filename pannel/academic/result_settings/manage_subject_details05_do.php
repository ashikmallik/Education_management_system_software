 ...
 require_once '../../../lib/htmlpurifier/library/HTMLPurifier.auto.php';
		...
		$con6= $_POST['con6'];
					  ...
					  if($term1==$term21)
					  {
									...
									for($i=0; $i<count($subid); $i++){
										...
										if($subid[$i]!=""){
											$insmk="UPDATE `borno_result_subject_details` SET `number_type1_marks`='$mark1[$i]',`number_type2_marks`='$mark2[$i]',`number_type3_marks`='$mark3[$i]',`number_type4_marks`='$mark4[$i]',`number_type5_marks`='$mark5[$i]',`number_type6_marks`='$mark6[$i]',`number_type1_pass`='$pass1[$i]',`number_type2_pass`='$pass2[$i]',`number_type3_pass`='$pass3[$i]',`number_type4_pass`='$pass4[$i]',`number_type5_pass`='$pass5[$i]',`number_type6_pass`='$pass6[$i]',`number_type1_conv`='$con1[$i]',`number_type2_conv`='$con2[$i]',`number_type3_conv`='$con3[$i]',`number_type4_conv`='$con4[$i]',`number_type5_conv`='$con5[$i]',`number_type6_conv`='$con6[$i]',`subject_type`='$subtype[$i]',`pare`='$pare[$i]' where borno_result_subject_details_id='$subdetailid[$i]'";
					  ...
					  elseif($term1!=$term21)
	{
		...
		for($i=0; $i<count($subid); $i++){
																	...
																	if($subid[$i]!=""){
											echo $insmk="INSERT INTO `borno_result_subject_details` (`borno_school_id`,`borno_school_class_id`,`borno_school_session`, `borno_result_term_id`,`borno_result_subject_id`, `number_type1_marks`,`number_type2_marks`,`number_type3_marks`,`number_type4_marks`,`number_type5_marks`,`number_type6_marks`,`number_type1_pass`,`number_type2_pass`,`number_type3_pass`,`number_type4_pass`,`number_type5_pass`,`number_type6_pass`,`number_type1_conv`,`number_type2_conv`,`number_type3_conv`,`number_type4_conv`,`number_type5_conv`,`number_type6_conv`,`subject_type`,`pare`)
											
											 VALUES ('$schId','$studClass1','$stsess1','$term21', '$subid[$i]', '$mark1[$i]','$mark2[$i]','$mark3[$i]','$mark4[$i]','$mark5[$i]','$mark6[$i]','$pass1[$i]','$pass2[$i]','$pass3[$i]','$pass4[$i]','$pass5[$i]','$pass6[$i]','$con1[$i]','$con2[$i]','$con3[$i]','$con4[$i]','$con5[$i]','$con6[$i]','$subtype[$i]','$pare[$i]')";
