		 ...
		 $ttmmrk=array_sum($perc);
					  ...
					  if($ttmmrk<100) { header('location:manage_term.php?msg=1'); }
									...
									for($i=0; $i<count($perc); $i++){
										...
										if($perc[$i]!=""){
											$insmk="UPDATE `borno_result_add_term_voc` SET `borno_result_term_name`='".trim($termName[$i])."',`borno_result_term_percent`='$perc[$i]' where `borno_result_term_id`='$termtype[$i]'";
