...
$this->SetFont('Arial','B',10); 
$this->Ln();
$this->Cell(200,5,"Expence Report",0,0,"C");
$this->Ln();
...
$this->Cell(35,5,"Expence Head",1,0,"L");
$this->Cell(35,5,"Expence Name",1,0,"L");
$this->Cell(30,5,"Expence By",1,0,"L");
...
$totalamount=0;		
$total = array();	
  $result ="select * from borno_school_expence where borno_school_id=$schId AND borno_school_ex_date between '$fdate' AND '$tdate' order by borno_school_expence_memo asc";
		$qresult=$mysqli->query($result);
		while($shgresult=$qresult->fetch_assoc()){
		$memo=$shgresult['borno_school_expence_memo'];
		
	
$gtreceipt ="select * from borno_school_expence where borno_school_expence_memo='$memo'";
					$qgstreceipt=$mysqli->query($gtreceipt);
					$streceipt=$qgstreceipt->fetch_assoc();
                    
        $stdate=$streceipt['borno_school_ex_date'];
		$stmon=$streceipt['borno_school_month'];
		$stexh=$streceipt['borno_school_exhead'];
		$stexn=$streceipt['borno_school_exname'];
		$stexby=$streceipt['borno_school_exby'];
		$stamount=$streceipt['borno_school_expence_amount'];
$total[]=$stamount;
