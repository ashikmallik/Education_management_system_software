    ...
    <td ><select name="monthid" id="monthid" onchange="callpage();">
      ...
      <?php } ?>
  		...
  		$gtduelist="SELECT `student_id`,`borno_school_student_name`,`due_amount` FROM `student_fees` AS sf 
                     ...
                     WHERE `fiscal_year_details_id` <= '$yeardetails' AND `fiscal_year_id`='$fiscalid' AND `borno_school_class_id` = '$studClass'  AND `due_amount` != 0 AND `fees_head_id` = 4
                     GROUP BY `student_id`";
		$sl=0;
...
<tr>
    ...
    <td width="12%;" align="center"><input name="prevDue[]" type="text" id="prevDue[]" size="1" tabindex=<?php echo $sl; ?> value="<?php echo $shgtduelist['due_amount']; ?>"></td>
