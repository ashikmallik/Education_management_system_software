...
*/
	...
	for($i=0; $i<count($stPhone); $i++){
			...
			$To=$stPhone[$i];
				...
				$gtstudent="select * from borno_school_group_number where borno_school_group_us_number='$stPhone[$i]'";
	$qgtstudent=$mysqli->query($gtstudent);
	while($shroll=$qgtstudent->fetch_assoc()){ 
	
			$name=$shroll['borno_school_group_us_name'];	
			
			$url = "http://api.rankstelecom.com/api/v3/sendsms/plain?user=mostafizbd&password=mostafizbd@01818306782&type=longSMS&sender=8804445629163&GSM=$To&SMSText=".  urlencode($message1);
	...
	if(!empty($response)) {
	  echo $response;
		...
		$bornosentSMS="INSERT INTO `borno_sent_sms` (`borno_school_id`,`borno_school_branch_id`,`borno_school_class_id`,`borno_school_shift_id`,`borno_school_section_id`,`borno_school_session`,`borno_school_roll`,`borno_school_student_name`, `borno_sent_sms_phone`,`borno_sms_status`,`borno_sms_type`, `borno_sent_sms_message`,`borno_sms_date`,`borno_sms_time`,`total_sms_count`,`bangla_message`) 
				VALUES ('$schId','0','0','0','0','0','0','$name', '$To','Group','$smsType', '$message1','$smsdate','$smstime','$totcountSMS1','$message1')";
